    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Choose how you want to add an upstream account.</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2">
          ...
          <button
            type="button"
            onClick={() => handleSelect(onAddFreemodel)}
            className={cn(
              "flex w-full cursor-pointer items-start gap-3 rounded-lg border p-3 text-left transition-colors hover:bg-muted/50",
              "outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]",
            )}
          >
            ...
            <span className="min-w-0">
              <span className="block text-sm font-medium">FreeModel API key</span>
              <span className="mt-0.5 block text-xs text-muted-foreground">
                Add a static API key for api.freemodel.dev
              </span>
            </span>
          </button>
        </div>
